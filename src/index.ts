...
export class Client {
  ...
  constructor(options: any = {}) {
    this.protocolVersion = options.protocolVersion || '1946277628';
    this.clientVersion = options.clientVersion || '12883';
    ...
    cookies.setCookie(request.cookie('Path=/'), 'https://us.draconiusgo.com');
    cookies.setCookie(
      request.cookie('domain=.draconiusgo.com'),
      'https://us.draconiusgo.com'
    );
  ...
  async getConfig(): Promise<objects.FConfig> {
    const config = await this.call('AuthService', 'getConfig', [
  ...
  async googleLogin() {
    ...
    this.auth.tokenId = await login.login(
      this.user.username,
      this.user.password
    );
  ...
  async getNews(lastSeen: string) {
    return await this.call('AuthService', 'getNews', [
      this.clientInfo.language,
      lastSeen
    ]);
  }
  ...
  async getMapUpdate(
    ...
    if (response.items) {
      const config = response.items.find(
        i => i.__type === 'FConfig'
      ) as objects.FConfig;
